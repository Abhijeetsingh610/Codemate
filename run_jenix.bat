@echo off
echo Codemate - AI Coding Assistant
echo =========================
echo.
...
echo 5. Start Codemate Pro Agent (Google API key or Ollama required)
...
echo.
if "%model%"=="" (
    python codemate.py --agent
) else (
    python codemate.py --model %model% --agent
)
echo.
pause
goto menu
...
echo.
if "%model%"=="" (
    python codemate.py --claude_agent
) else (
    python codemate.py --model %model% --claude_agent
)
echo.
pause
goto menu
...
echo.
if "%model%"=="" (
    python codemate.py --claude_agent --local
) else (
    python codemate.py --model %model% --claude_agent --local
)
echo.
pause
goto menu
...
:codemate_pro
cls
echo Starting Codemate Pro Agent...
echo.
...
if /i "%use_ollama%"=="y" (
    ...
    set /p model="Enter model name (or press Enter for default): "
    echo.
    call :colorEcho %GREEN% "Starting Codemate Pro Agent with local model..."
    echo.
    if "%model%"=="" (
        python codemate.py --codemate_pro_agent --use-ollama
    ) else (
        python codemate.py --model %model% --codemate_pro_agent --use-ollama
    )
) else (
    call :colorEcho %GREEN% "Starting Codemate Pro Agent with Gemini..."
    echo.
    python codemate.py --codemate_pro_agent
)
echo.
pause
goto menu
...
echo.
python codemate.py --list-models
echo.
pause
goto menu
